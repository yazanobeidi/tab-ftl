...
set_random_seed(42)
from cover.dataset import load_covertype
from cover.federated import FederatedTrainer

class CoverTrainer(FederatedTrainer):
    def __init__(self, run_description, batches, ee_dim, seed, 
        ...
        self.trains, self.tests, self.vals = load_covertype(self.batches,
                                                            drop_last=self.drop_last,
                                                            seed=self.seed,
                                                            device=device,
                        force_consistent_target_space=force_consistent_target_space)
        self.covertype_model_loader = import_model_loader(model_filename)
        self.models = self.covertype_model_loader(self.trains, ee_dim=ee_dim, seed=self.seed,
                                                  personalize=personalize, device=device)
        self.names = {ix:name for ix, name in enumerate(self.models.keys())}
        self.loss_fn = [torch.nn.CrossEntropyLoss() for _ in self.models]
        self.optimizer = [torch.optim.AdamW(m.parameters(), lr=0.001, 
                                 weight_decay=0.0001) for m in self.models.values()]
        ...
        for i, data in enumerate(zip(*list(self.trains.values()))):
            for ix in torch.randperm(len(data)):
                ...
                try:
                    ...
                    print('error on', name, outputs, labels, self.trains[name].dataset.y.unique())
                    raise
        ...
        for split, train_metric in train_metrics.items():
            for train, val in zip(train_metric.items(), val_metrics[split].items()):
                ...
                try:
                    if 'conf' in name:
                        continue
                        # train_metric = train_metric.tolist()
    ...
    def test(self):
            ...
            for name, metric in test_metric.items():
                try:
                    if 'conf' in name:
                        ...
                        # classes = [0]+self.tests[split].dataset.get_unique_classes().tolist()
                        # print(classes)
        ...
        path_to_model_def = self.covertype_model_loader.__globals__["__file__"]
